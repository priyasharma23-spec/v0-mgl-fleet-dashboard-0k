      ...
      {selectedDriver && (
        ...
        >
          ...
          {detailTab === "policy" && selectedDriver && (() => {
            const expiryOpts = [{ label:"2h",value:2},{label:"24h",value:24},{label:"72h",value:72},{label:"7 days",value:168},{label:"No expiry",value:null}]
            const triggerOpts = [{id:"on_vehicle_change",label:"On vehicle change",desc:"New code when vehicle changes"},{id:"monthly",label:"Monthly",desc:"Auto-regenerates on 1st of month"},{id:"manual",label:"Manual only",desc:"FO manually regenerates"}]
            const pol = draftPolicy ?? {codeType:"single_use",expiryHours:24,maxUsesPerCode:1,repairingTrigger:"on_vehicle_change"}
            const risk = pol.maxUsesPerCode===1&&pol.expiryHours&&pol.expiryHours<=24 ? {label:"High security",color:"bg-green-50 text-green-700 border-green-200"} : pol.maxUsesPerCode===null&&pol.expiryHours===null ? {label:"Open access",color:"bg-red-50 text-red-700 border-red-200"} : {label:"Balanced",color:"bg-amber-50 text-amber-700 border-amber-200"}
            ...
            return (
              <div className="space-y-4 mt-4">
                <div className="grid grid-cols-2 gap-2">
                  {[{id:"vehicle_linked",label:"Vehicle-linked"},{id:"shift_based",label:"Shift-based"},{id:"trip_linked",label:"Trip-linked"},{id:"pairing_code",label:"Code override"}].map(mode => (
                    <button key={mode.id} onClick={() => { setActivePolicyMode(mode.id as any); setDraftPolicy({codeType:"single_use",expiryHours:24,maxUsesPerCode:1,repairingTrigger:"on_vehicle_change"}) }}
                        ...
                        {driverBindings.map(b=>{
                          ...
                          return(
                            <div key={b.id} className="flex items-center justify-between p-3 rounded-xl border border-border bg-card">
                              <div><p className="font-mono font-bold text-sm tracking-widest">{b.pairingCode??"—"}</p><p className="text-xs text-muted-foreground mt-0.5">{v?.vehicleNumber??b.vehicleId}</p></div>
                              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${b.pairingCodeState==="used"?"bg-green-50 text-green-700":"bg-amber-50 text-amber-700"}`}>{b.pairingCodeState??"pending"}</span>
                            </div>
                          )
                        })}
                      </div>
                  ...
                  <div className="flex gap-2 pt-1">
                    {[{id:"single_use",label:"Single use",desc:"One fueling per code"},{id:"multi_use",label:"Multi use",desc:"Reusable until expiry"}].map(opt=>(
                      <button key={opt.id} onClick={()=>setDraftPolicy(p=>p?{...p,codeType:opt.id as any,maxUsesPerCode:opt.id==="single_use"?1:p.maxUsesPerCode}:p)}
                        className={`flex-1 p-3 rounded-xl border text-left transition-all ${pol.codeType===opt.id?"border-primary bg-primary/5":"border-border hover:border-primary/50"}`}>
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    {expiryOpts.map(opt=>(
                      <button key={String(opt.value)} onClick={()=>setDraftPolicy(p=>p?{...p,expiryHours:opt.value}:p)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${pol.expiryHours===opt.value?"bg-primary text-primary-foreground border-primary":"bg-background border-border hover:border-primary/50"}`}>
                        {opt.label}
                      </button>
                    ))}
                  </div>
                ...
                <TraySection title="Max Uses Per Code">
                  <div className="flex flex-wrap gap-2 pt-1">
                    {[1,3,5,10,null].map(n=>(
                      <button key={String(n)} disabled={pol.codeType==="single_use"&&n!==1}
                        onClick={()=>setDraftPolicy(p=>p?{...p,maxUsesPerCode:n}:p)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${pol.maxUsesPerCode===n?"bg-primary text-primary-foreground border-primary":pol.codeType==="single_use"&&n!==1?"opacity-30 cursor-not-allowed border-border":"bg-background border-border hover:border-primary/50"}`}>
                        {n===null?"Unlimited":n}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="space-y-2 pt-1">
                    {triggerOpts.map(opt=>(
                      <button key={opt.id} onClick={()=>setDraftPolicy(p=>p?{...p,repairingTrigger:opt.id}:p)}
                        className={`w-full p-3 rounded-xl border text-left transition-all flex items-center gap-3 ${pol.repairingTrigger===opt.id?"border-primary bg-primary/5":"border-border hover:border-primary/50"}`}>
                        <div className="flex-1"><p className="text-xs font-medium">{opt.label}</p><p className="text-[10px] text-muted-foreground mt-0.5">{opt.desc}</p></div>
                        {pol.repairingTrigger===opt.id&&<div className="w-4 h-4 rounded-full bg-primary flex items-center justify-center"><svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3"><path d="M20 6L9 17l-5-5"/></svg></div>}
                      </button>
                    ))}
                  </div>
                </TraySection>

...
export function RightTray({ open, onClose, title, subtitle, children, footer, badge, headerActions, zIndex = "z-50", accentColor }: RightTrayProps) {
  if (!open) return null
  return (
    <>
      <div className="fixed inset-0 bg-black/50 z-40" onClick={onClose} />
      <div className={`fixed bottom-0 right-0 top-0 w-96 bg-card border-l border-border shadow-xl flex flex-col ${zIndex} ${accentColor ?? ""}`}>
        <div className="sticky top-0 bg-card border-b border-border p-4 shrink-0">
